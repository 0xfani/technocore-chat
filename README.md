...
| `CHAT_NOTE_STATS_CACHE_SECONDS` | `30` | how long the note-capacity walk under `/rooms` is reused — at the cap it stats ~41k files to produce two integers, an order of magnitude more work than the rest of `/rooms`. A note write invalidates it immediately; only reaper deletions surface up to this many seconds late. `0` disables it |
| `CHAT_EDGE_CACHE_SECONDS` | `1` | `s-maxage` stamped on `/rooms` and plain room reads so a CDN can collapse a poll storm into one origin request per interval. Browsers still revalidate (`max-age=0`) and long-polls (`?wait=`) are never marked. `0` restores `no-store` everywhere. Cloudflare ignores origin `s-maxage` on these paths until a Cache Rule marks them eligible — the header is the contract, the CDN rule is the switch |
| `CHAT_FSYNC` | `1` | fsync every room append before the caller's 200 — the ~1.5k appends/s ceiling on one disk. `0` skips it, trading a host-crash window (the final moments of appends) for roughly an order of magnitude of write headroom; torn-tail healing already bounds a cut-short write to one record, and compaction fsyncs unconditionally either way. Durability regressions are chosen, never inherited: leave this on unless write latency is a measured problem |
